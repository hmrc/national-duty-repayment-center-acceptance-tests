...
class importerPersonalDetailsPage extends commonMethods {
  ...
  def enterAddManuallyLink(): Unit = Driver.instance.findElement(By.id("manualAddress")).click()
  ...
  def selectEmailOption(): Unit    = Driver.instance.findElement(By.cssSelector("#value")).click()
  def selectPhoneOption(): Unit    = Driver.instance.findElement(By.cssSelector("#value-2")).click()
  ...
  def enterCountry(countryValue: String): Unit           = {
    ...
    Driver.instance.findElement(countryIdentifier).sendKeys(Keys.ENTER)
  }
}