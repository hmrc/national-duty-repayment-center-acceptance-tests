...
class UKRegulationsSpec extends BaseSpec {

  Feature("UK Regulations scenarios") {

    Scenario(
      "A user wants to complete a New Importers journey with UK regulations [YorNvat=Yes, fName=Test, lName=Importer, YorNgoodsOwner=No, goodsOwner=Goods Owner, phoneNo=09876543212, emailAddress=test@mail.com, YorNDeclarantRefNO=Yes, declarantRefNo=123abcxyz]"
    ) {
      ...
      andIChooseRepaymentOption(
        "Current month amendment (CMA)"
      ) // auto-chosen (score=0.70, selectRepayMethodStepsSteps.scala)
      // --- Other possible matches ---
      // andIChooseOption() [0.70] (VatRegStepsSteps.scala) pattern: I choose "([^"]*)" option
      // andIChooseEori() [0.52] (EoriNoStepsSteps.scala) pattern: I choose eori "([^"]*)" option
      // andIChooseRepresentativeOption() [0.52] (whoToBoRepaidStepsSteps.scala) pattern: I choose Representative option
    ...
    Scenario("UK regulations available for entry date 1 Jan 2021") {
      ...
      andIEnterAnEntryDateOfX("1", "1", "2021") // auto-chosen (score=1.00, ImporterPersonalDetailsStepsSteps.scala)
      // --- Other possible matches ---
    ...
    Scenario("Customs articles available for entry date pre 1 Jan 2021") {
      ...
      andIEnterAnEntryDateOfX("31", "12", "2020") // auto-chosen (score=1.00, EntryDetailsStepsSteps.scala)
      // --- Other possible matches ---
      // andIEnterAnEntryDateOfX() [1.00] (EntryDetailsStepsSteps.scala) pattern: I enter an entry date of (.*)/(.*)/(.*)
      // andIEnterCountry() [0.47] (ImporterPersonalDetailsStepsSteps.scala) pattern: I enter country \"([^\"]*)\"
      // andIEnterPhoneNo() [0.47] (ImporterPersonalDetailsStepsSteps.scala) pattern: I enter \"([^\"]*)\" importers
