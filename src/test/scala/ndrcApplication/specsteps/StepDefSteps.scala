...
object StepDefsSteps extends commonMethods {
  ...
  // ^I verify correct responses is displayed for Amend case journey \"([^\"]*)\", \"([^\"]*)\", \"([^\"]*)\"$
  def thenIVerifyCorrectResponsesIsDisplayedForAmendCaseJourneyMultiple(
    refNo: String,
    resType: String,
    documents: String*
  ): Unit = {
  ...
  def andIVerifyChangedOtherDutyShouldHavePaidIsDisplayed(changeODutyShldHavePaid: String): Unit =
