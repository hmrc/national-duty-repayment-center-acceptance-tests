...
class RepaymentAmountSummarySteps extends repaymentAmtSummaryPage with ScalaDsl with EN {
  ...
  And("^I clear amount and enter customs Duty should have paid- CustomsDutyBeenPaid: \"([^\"]*)\"$") { (changeCustDutyShldHavePaid: String) =>
    customsDutyPage.clearCustDutyShldHavePaid
    customsDutyPage.enterCustDutyHaveBeenPaid(changeCustDutyShldHavePaid)
  }

  And("^I verify changed customs duty should have paid- CustomDuty: \"([^\"]*)\" is displayed$") { (changeCDutyShldHavePaid: String) =>
  ...
  And("^I clear amount and enter import vat been Paid to HMRC- VatBeenPaid: \"([^\"]*)\"$") { (changeImpVatShldHavepaid: String) =>
    vatPage.clearImpVATBeenPaid
    vatPage.enterVatBeenPaid(changeImpVatShldHavepaid)
  }

  And("^I verify changed import vat been Paid to HMRC- VatBeenPaid: \"([^\"]*)\" is displayed$") { (changeIVatShldHavepaid: String) =>
  ...
  And("^I clear amount and enter other duty should have paid- OtherDutyBeenPaid: \"([^\"]*)\"$") { (changeOtherDutyShldHavePaid: String) =>
    otherCustDutyPage.clearOtherDutyBeenPaid
    otherCustDutyPage.enterOtherCustDutyBeenPaid(changeOtherDutyShldHavePaid)
  }

  And("^I verify changed other duty paid- OtherDutyBeenPaid: \"([^\"]*)\" is displayed$") { (changeODutyShldHavePaid: String) =>
